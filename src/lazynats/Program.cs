...
/// <param name="user">-u, Username or Token.</param>
/// <param name="password">-p, Password.</param>
/// <param name="token">-t, Token.</param>
/// <param name="fps">Frames per second.</param>
static async Task RunAppAsync(
    string? server = null,
    string? user = null, string? password = null, string? token = null,
    int? fps = null)
{
    var (resolvedUser, resolvedPassword, resolvedToken) = ResolveAuth(user, password, token);
    var authOpts = BuildNatsAuthOpts(resolvedToken, resolvedUser, resolvedPassword);
    ...
    Application.MaximumIterationsPerSecond = (ushort)(fps ?? 30).NotLessThan(1).NotMoreThan(60);
    ...
    if (app.Popovers != null)
    {
        app.Popovers.PopoverRegistered += (_, e) => {
            ...
            if (popover.Anchor?.Invoke()?.Width is { } width && popover.ContentView != null)
                popover.ContentView.Width = width;
        };
    }
...
static NatsAuthOpts? BuildNatsAuthOpts(string? token, string? user, string? password) =>
    token is not null ? new NatsAuthOpts { Token = token } :
    user is not null && password is not null ? new NatsAuthOpts { Username = user, Password = password } :
    user is not null ? new NatsAuthOpts { Token = user } :
    null;
...
static void ApplyColorTheme()
{
    ...
    SchemeManager.AddScheme(
        "Base", new Scheme(SchemeManager.GetScheme("Base")) {
            Normal = normal,
            Editable = editable,
        });

    SchemeManager.AddScheme(
        "Dialog", new Scheme(SchemeManager.GetScheme("Dialog")) {
            Normal = normal,
