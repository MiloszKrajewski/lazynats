...
chord. It SHALL NOT list the application's hardcoded top-level shortcuts (`Alt-1..5`, `Alt-P`,
`Alt-Q`, `?` itself) — those are already permanently visible in the status bar.
...
- **WHEN** the picker is open and the user presses one of the application's hardcoded top-level
  shortcut keys (e.g. `Alt-1..5`, `Alt-P`, `Alt-Q`, or the picker's own `?` trigger)
