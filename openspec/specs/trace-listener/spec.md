# trace-listener Specification

## Purpose
Give developers a debug-output channel for `lazynats` internals - which have no `Console.WriteLine`
reachable (the TUI owns the screen) and mostly no DI seam to inject a logger into - by forwarding
`System.Diagnostics.Trace` calls into the existing Live Feed pane as synthetic `$TRACE`-subject
messages, entirely absent from Release/AOT builds.
