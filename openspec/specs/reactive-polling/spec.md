# reactive-polling Specification

## Purpose
Provide reusable, App-thread-safe Rx-based building blocks (`Core/AsyncExtensions.cs`'s
`SelectAsync`/`ObserveOnApp` operators, plus the active-gated always-running poll-timer pattern
built on them) for components that need to poll the server on a fixed interval and update UI
state safely, without duplicating ad-hoc `App?.Invoke(...)` marshaling or hand-rolled staleness
guards in each such component.
