# shortcut-picker-launch Specification

## Purpose
Defines the reusable mechanism for opening the shortcut picker: how a caller supplies (or omits)
the focus-chain start view, the reusable-action and direct-key-binding forms callers can attach
the picker-opening sequence to, and the single source of truth for the picker's trigger key.
