# payload-edit-section Specification

## Purpose

Define the shared editable Payload Type + payload/value editing view (`Components/PayloadEditSection`)
composed identically by every dialog that composes or edits a message payload or KV value (Publish,
Create/Edit Template, Create/Edit Key): its layout (fixed type row, fill-the-rest editor row),
word-wrap behavior, invalid-state highlighting, encoded-bytes access, and the one seeding-from-bytes
rendering rule used by every caller. This is the editable counterpart to `payload-presentation`'s
read-only `PayloadDetailSection`.

## Requirements

### Requirement: Shared Editable Payload Section
The system SHALL provide a single reusable view, composed identically by every dialog that composes
or edits a message payload or KV value (Publish, Create/Edit Template, Create/Edit Key), consisting
of a Payload Type selector and a payload text editor, so that Payload Type/Payload editing behavior
(validation, wrap behavior, invalid-state highlighting, and byte encoding) is defined once rather
than independently by each dialog.
