# cli-server-url Specification

## Purpose

Lets a user point `lazynats` at a NATS server other than the default `nats://localhost:4222`
without editing configuration files, via a `NATS_URL` environment variable or a `--server`/`-s`
command-line option, with a clear precedence between the two.
