...
## Purpose
Provide a read-only "Streams" management tab that lists JetStream streams on the connected
server and shows configuration/state detail for the highlighted stream, and lets the user drill
down from a stream into its consumers to inspect their configuration and delivery/ack state, so
the user can inspect JetStream state without leaving the terminal.
