# quit-key Specification

## Purpose

How the app quits: an Esc that reaches the main window unhandled (nothing left to go "back"
from) is swallowed rather than quitting, so over-pressing Esc while backing out cannot throw
away the session. Alt+Q, advertised in the status bar, is the explicit one-press quit.
