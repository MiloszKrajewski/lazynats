# cli-connection-auth Specification

## Purpose

Lets a user authenticate `lazynats` against a NATS server that requires username/password or
token authentication, without editing code, via `--user`/`--password`/`--token` command-line
options or matching `NATS_USER`/`NATS_PASSWORD`/`NATS_TOKEN` environment variables — mirroring
the `nats` CLI's own auth flags and the precedence already established by `cli-server-url`.
